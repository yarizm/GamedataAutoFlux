...
LangGraph 驱动（general 路径用 `langchain.agents.create_agent`），是最复杂的子系统：

- **`agent.py`** — `AgentService` 核心，管理 Agent 生命周期、会话、工具注册。
- **`runtime.py`** — LangGraph 单运行时（`LangGraphAgentRuntime`）；已移除 `langchain_classic` / AgentExecutor。
...
- **Scheduler 只作执行引擎**（调度/并发/生命周期）。`PipelineService`（src/services/pipeline_service.py）是 Pipeline 注册表的**所有者**（内存注册/持久化/按名解析/删除，依赖 callables 注入、零 Scheduler 依赖），由 Scheduler 组合持有，Scheduler 上的同名方法只是兼容 shim；Cron 门面直连内部 `SchedulerCronService`（`scheduler.cron_service`）；Worker claim/complete/fail 属执行协议，留在 Scheduler。Web/Agent 业务操作经容器 getter（`get_pipeline_service()` 等），禁止直呼 Scheduler 业务方法（`tests/test_no_web_reverse_deps.py` 静态校验）。
