...
class VectorizerProcessor(BaseProcessor):
    ...
    async def process_batch(self, inputs: list[ProcessInput]) -> list[ProcessOutput]:
        ...
        if not self.embeddings:
            ...
            return [ProcessOutput(success=True, data=p_in.data, metadata=p_in.metadata, processor_name="vectorizer") for p_in in inputs]
        ...
        for p_in in inputs:
            if not p_in.data:
                results.append(ProcessOutput(success=True, data=p_in.data, metadata=p_in.metadata, processor_name="vectorizer"))
                continue
